  ...
  BEGIN
    ...
    LOOP
      INSERT INTO STUFF_S (ID_STUFF, ID_POSITION, NAME, SURNAME, EMAIL) VALUES (STUFF_REC.ID_STUFF, 1,
                                                                                REGEXP_SUBSTR(STUFF_REC.STAFF_NAME,
                                                                                              '[^ ]+', 1, 1),
                                                                                REGEXP_SUBSTR(STUFF_REC.STAFF_NAME,
                                                                                              '[^ ]+', 1, 2),
                                                                                STUFF_REC.E_MAIL);
...
CREATE OR REPLACE PROCEDURE LOAD_SUPPLIER IS
  CURSOR C2 IS
    SELECT DISTINCT SUPPLIER
    FROM INVOICE
    WHERE REGEXP_LIKE(SUPPLIER, '^[A-Za-z]');
  ID_NUM INTEGER := 1;
  BEGIN
...
CREATE OR REPLACE PROCEDURE LOAD_PRODUCT IS
  CURSOR C3 IS
    SELECT DISTINCT
      ...
      PRICE
    FROM INVOICE
    WHERE REGEXP_LIKE(SUPPLIER, '^[A-Za-z]') AND PRODUCT LIKE 'TV-__';
  ID_NUM INTEGER := 1;
  ID_SUP INTEGER;
  BEGIN
    ...
    LOOP
      ...
      INTO ID_SUP
      FROM SUPPLIER_S
      WHERE SUPPLIER_NAME = PRODUCT_REC.SUPPLIER;
...
CREATE OR REPLACE PROCEDURE LOAD_INVOICE IS
  ...
  BEGIN
    ...
    LOOP
      IF INVOICE_REC.QUANTITY BETWEEN 0 AND 100
      THEN
...
CREATE OR REPLACE PROCEDURE LOAD_INVOICE_DETAILS IS
  CURSOR C5 IS
    SELECT DISTINCT
      REGEXP_SUBSTR(INVOICE, '[^-]+', 1, 2) AS ID_INVOICE,
      QUANTITY,
      SUPPLIER,
      PRODUCT,
      PRICE
  ...
  BEGIN
    ...
    LOOP
      ...
      THEN
        ...
        INTO ID_P
        FROM PRODUCT_S T1
          JOIN SUPPLIER_S T2 ON (T1.ID_SUPPLIER = T2.ID_SUPPLIER)
        WHERE SUPPLIER_NAME = INVOICE_REC.SUPPLIER
...
CREATE OR REPLACE PROCEDURE LOAD_PRODUCT_STORE IS
  CURSOR C6 IS
    SELECT DISTINCT
      ...
      PRODUCT,
      PRODUCT_NAME
    FROM STORE T1 LEFT JOIN PRODUCT_S T2 ON (T1.PRODUCT = T2.PRODUCT_NAME)
    WHERE SUPPLIER IN (SELECT SUPPLIER_NAME
                       FROM SUPPLIER_S)
  ...
  BEGIN
    ...
    FOR P_REC IN C6
    LOOP
      IF P_REC.PRODUCT_NAME IS NULL
      THEN
        SELECT ID_SUPPLIER
        INTO ID_S
        FROM SUPPLIER_S
        WHERE SUPPLIER_NAME = P_REC.SUPPLIER;
...
CREATE OR REPLACE PROCEDURE LOAD_STORE IS
  CURSOR C7 IS
    SELECT
      ...
      OPER_TYPE
    FROM STORE
    WHERE REGEXP_SUBSTR(STUFF_NAME, '[^ ]+', 1, 1) IN (SELECT NAME
                                                       FROM STUFF_S) AND REGEXP_SUBSTR(STUFF_NAME, '[^ ]+', 1, 2) IN
                                                                         (SELECT SURNAME
                                                                          FROM STUFF_S) AND
          SUPPLIER IN (SELECT SUPPLIER_NAME
                       FROM SUPPLIER_S) AND SHELF < 100 AND
          REGEXP_LIKE(STORE_DATE, '^(0?[1-9]|[12][0-9]|3[01])\.(0?[1-9]|1[012])\.\d{4}$') AND
          PRODUCT IN (SELECT PRODUCT_NAME
                      FROM PRODUCT_S) AND UPPER(OPER_TYPE) IN (SELECT NAME_OPER
                                                               FROM TYPE_OPER_S);
  ID_P   NUMBER;
  ID_S   NUMBER := 1;
  ID_SUP NUMBER;
  ...
  BEGIN
    ...
    LOOP
      IF IS_NUMBER(INVOICE_REC.QUANTITY) = 1 AND INVOICE_REC.QUANTITY BETWEEN 0 AND 100 AND
         IS_DATE(INVOICE_REC.STORE_DATE) = 1
      THEN
        DBMS_OUTPUT.PUT_LINE('WTF');
        SELECT ID_SUPPLIER
        INTO ID_SUP
        FROM SUPPLIER_S
        WHERE SUPPLIER_S.SUPPLIER_NAME = INVOICE_REC.SUPPLIER;
        ...
        INTO ID_OP
        FROM TYPE_OPER_S
        WHERE NAME_OPER = UPPER(INVOICE_REC.OPER_TYPE);
        DBMS_OUTPUT.PUT_LINE(
            'ID_S' || ID_S || ' ID_P' || ID_P || ' SHELF' || INVOICE_REC.SHELF || ' DATE' || INVOICE_REC.STORE_DATE);
...
USING (
        SELECT DISTINCT
          ...
          T1.ID_STUFF,
          T1.PURCHASE_TIME
        FROM INVOICE_S T1 LEFT JOIN INVOICE_D T2 ON
                                                   (T1.ID_INVOICE = T2.ID_INVOICE)) SOURCE
ON (TARGET.ID_INVOICE = SOURCE.ID_INVOICE)
WHEN MATCHED THEN
...
USING (
        SELECT DISTINCT
          ...
          T1.DESCRIPTION,
          T1.PRICE
        FROM PRODUCT_S T1 LEFT JOIN PRODUCT_D T2 ON
                                                   (T1.ID_PRODUCT = T2.ID_PRODUCT)) SOURCE
...
MERGE INTO INVOICE_DETAIL_D TARGET
USING (
        SELECT DISTINCT
          T1.ID_INVOICE,
          T1.ID_PRODUCT,
          T1.QUANTITY,
          T1.DESCRIPTION
        FROM INVOICE_DETAIL_S T1 LEFT JOIN INVOICE_DETAIL_D T2 ON
                                                                 (T1.ID_INVOICE = T2.ID_INVOICE)) SOURCE
ON (TARGET.ID_INVOICE = SOURCE.ID_INVOICE)
WHEN MATCHED THEN
...
USING (
        ...
        FROM STORE_S T1
          LEFT JOIN STORE_D T2 ON
                                 (T1.ID_STORE = T2.ID_STORE)) SOURCE
ON (TARGET.ID_STORE = SOURCE.ID_STORE)
WHEN MATCHED THEN
  UPDATE SET ID_PRODUCT = SOURCE.ID_PRODUCT, SHELF = SOURCE.SHELF, DATE_OPER = SOURCE.DATE_OPER,
    ID_OPER_TYPE        = SOURCE.ID_OPER_TYPE,
    QUANTITY            = SOURCE.QUANTITY
WHEN NOT MATCHED THEN
  INSERT
  VALUES (SOURCE.ID_STORE, SOURCE.ID_PRODUCT, SOURCE.SHELF, SOURCE.DATE_OPER, SOURCE.ID_OPER_TYPE, SOURCE.QUANTITY);

MERGE INTO STUFF_D TARGET
USING (
        SELECT DISTINCT
          T1.ID_STUFF,
          T1.ID_POSITION,
        ...
        FROM STUFF_S T1
          LEFT JOIN STUFF_D T2 ON
                                 (T1.ID_STUFF = T2.ID_STUFF)) SOURCE
ON (TARGET.ID_STUFF = SOURCE.ID_STUFF)
WHEN MATCHED THEN
...
WHEN NOT MATCHED THEN
  INSERT
  VALUES (SOURCE.ID_STUFF, SOURCE.ID_POSITION, SOURCE.NAME, SOURCE.SURNAME, SOURCE.PHONE, SOURCE.ADDRESS, SOURCE.EMAIL);

MERGE INTO SUPPLIER_D TARGET
USING (
        SELECT DISTINCT
          T1.ID_SUPPLIER,
          T1.SUPPLIER_NAME,
        ...
        FROM SUPPLIER_S T1
          LEFT JOIN SUPPLIER_D T2 ON
                                    (T1.ID_SUPPLIER = T2.ID_SUPPLIER)) SOURCE
...
USING (
        ...
        FROM TYPE_OPER_S T1
          LEFT JOIN TYPE_OPER_D T2 ON
                                     (T1.ID_OPER_TYPE = T2.ID_OPER_TYPE)) SOURCE
ON (TARGET.ID_OPER_TYPE = SOURCE.ID_OPER_TYPE)
WHEN MATCHED THEN
