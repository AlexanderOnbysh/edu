...
select i.id_stuff, nvl(to_char(idl.id_product),'хрнцн:'), sum(idl.quantity) from invoice i, invoice_detail idl
...
select i.id_stuff,i.id_type, nvl(to_char(idl.id_product),'хрнцн:'), sum(idl.quantity) from invoice i, invoice_detail idl
where i.id_invoice = idl.id_invoice
